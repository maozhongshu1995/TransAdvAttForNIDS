...
def main(dataset_name, sm_name, tm_name, attack, iteration, step_size):
    # hyper
    dev = torch.device('cuda')
    batch_size = 128

    fp = os.path.join(project_root_dir, 'storage', 'AAT', sm_name, attack, f'{iteration}_{step_size}.csv')
    ...
    for flows, labels in dataloader:
        ...
        acc = TP / (TP + FN)
        print(f"\r {sm_name}->{tm_name}, {attack}, {iteration}-{step_size}, Progress:{curr_iter}/{len(dataset)} Acc: {acc:.3f}", end="")
    print()
    return acc
...
if __name__ == '__main__':
    ...
    for tm_name in tm_names:
        for step_size in step_sizes:
            list_midx.append((tm_name, step_size))
    midx = pd.MultiIndex.from_tuples(list_midx, names=['Model', 'step_size'])
    df = pd.DataFrame([[0.] * len(iterations)] * len(list_midx), index=midx, columns=iterations)
    print(df)

    for tm_name in tm_names:
        for step_size in step_sizes:
            for iteration in iterations:
                acc = main(dataset_name, sm_name, tm_name, attack, iteration, step_size)
                df.loc[(tm_name, step_size), iteration] = round(acc * 100, 1)
        print(df)

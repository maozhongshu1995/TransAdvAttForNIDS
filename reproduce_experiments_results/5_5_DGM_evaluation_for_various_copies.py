...
if __name__ == '__main__':
    ...
    for onemodel in sm_names:
        for onecopy in copies:
            list_midx.append((onemodel, onecopy))

    midx = pd.MultiIndex.from_tuples(list_midx, names=['Model', 'Copies'])
    df = pd.DataFrame([[0.] * len(tm_names)] * len(list_midx), index=midx, columns=tm_names)

    print(df)
    for sm_name in sm_names:
        for num_copy in copies:
            acc_sum = 0
            for tm_name in tm_names:
                acc = main(dataset_name, sm_name, tm_name, attack, num_copy, dropout_rate)
                df.loc[(sm_name, num_copy), tm_name] = round(acc * 100, 1)
                acc_sum += acc
            df.loc[(sm_name, num_copy), 'Average'] = round((acc_sum / len(tm_names)) * 100, 1)
        print(df)

...
def main(dataset_name, model_name, model_type, fp_model, fp_final_att, fp_fea, fp_minmax):

    dev = torch.device('cuda')
    batch_size = 128

    dataset = CustomDataset(fp_final_att, fp_minmax, fp_fea)
    ...
    for flows, labels in dataloader:
        ...
        acc = (TP / (TP + FN)) * 100
        print(f"\rProgress:{curr_iter}/{len(dataset)} Acc: {acc:.1f}", end="")
    print()
...
if __name__ == '__main__':
    ...
    fp_final_att = os.path.join(project_root_dir, 'output2', 'final_aat.csv')
    ...
    main(dataset_name, model_name, model_type, fp_model, fp_final_att, fp_fea, fp_minmax)
